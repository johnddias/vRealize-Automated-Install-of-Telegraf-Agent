...
def configureTelegrafPlugin(context, inputs):
    ...
    r = requests.request('POST','https://'+vropsHost+'/suite-api/api/applications/agents/'+inputs["resId"]+'/services', data=inputs["pluginConfig"], headers=headers, verify=inputs["verify"])
    print(r.status_code)
    print(r.url)
